...
bool sys_structseq_reduce_payload(
    Runtime& runtime,
    const Value& self,
    Value& out,
    std::string& error,
    const SysStructSeqMethodData* data,
    const char* method_name) {
  const std::string owner_name = data != nullptr ? data->owner_name : "sys.structseq";
  auto* instance = value_as_instance(self);
  TupleObject* tuple = nullptr;
  if (instance == nullptr || !sys_structseq_tuple_storage(self, method_name, tuple, error)) {
    error = "descriptor '" + std::string(method_name) + "' for '" + owner_name +
            "' objects doesn't apply to a '" + sys_type_name(runtime, self) + "' object";
    runtime.raise_class_error("TypeError", error);
    return false;
  }
  ...
  Value names_value;
  TupleObject* names = nullptr;
  if (object_get_attr(self, "_all_field_names", names_value, error) &&
      (names = value_as_tuple(names_value)) != nullptr) {
    for (size_t i = tuple->items.size(); i < names->items.size(); ++i) {
      ...
      if (object_get_attr(self, field_name, field_value, ignored)) {
...
bool sys_structseq_reduce(Runtime& runtime, const Value* args, uint32_t argc, Value& out, std::string& error, void* user_data) {
  ...
  return sys_structseq_reduce_payload(runtime, args[0], out, error, data, "__reduce__");
}

bool sys_structseq_reduce_ex(Runtime& runtime, const Value* args, uint32_t argc, Value& out, std::string& error, void* user_data) {
  auto* data = static_cast<SysStructSeqMethodData*>(user_data);
  if (argc == 0) {
  ...
  if (args[1].tag != ValueTag::Int64 && args[1].tag != ValueTag::Bool) {
    error = "'" + sys_type_name(runtime, args[1]) + "' object cannot be interpreted as an integer";
    runtime.raise_class_error("TypeError", error);
    return false;
  }
  return sys_structseq_reduce_payload(runtime, args[0], out, error, data, "__reduce_ex__");
}
...
bool sys_structseq_reduce_ex_kw(
    Runtime& runtime,
    const Value*,
    uint32_t,
    const NativeKeywordArg*,
...
Value make_sys_structseq_method(
    ...
    const char* text_signature = "($self, /)") {
  ...
  if (auto* native = value_as_native_function(function)) {
    native->attrs_dict = new Value(Value::dict({
        ...
        {Value::string("__doc__"), std::move(doc)},
...
Value make_structseq(
  ...
  class_attrs.push_back({"__reduce_ex__", make_sys_structseq_method(
                                            runtime,
                                            type_name,
                                            "__reduce_ex__",
                                            sys_structseq_reduce_ex,
                                            new SysStructSeqMethodData{
                                                type_name,
                                                module_name.empty() ? type_name : module_name + "." + type_name},
                                            sys_structseq_method_data_cleanup,
                                            sys_structseq_reduce_ex_kw,
                                            Value::string("Helper for pickle."),
                                            "($self, protocol, /)")});
...
Value make_builtin_module_names() {
  return Value::tuple({
      ...
      Value::string("_codecs_iso2022"),
      Value::string("_codecs_jp"),
      Value::string("_codecs_kr"),
