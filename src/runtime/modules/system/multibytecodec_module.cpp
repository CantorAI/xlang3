...
#ifndef _WIN32
const char* encoding_from_code_page(int code_page) {
  switch (code_page) {
...
bool iconv_convert(const char* from_encoding, const char* to_encoding,
  ...
  bool ok = true;
  while (true) {
    char* output_ptr = buffer.data() + output_used;
    size_t output_left = buffer.size() - output_used;
    const size_t result = iconv(converter, &input_ptr, &input_left,
                                &output_ptr, &output_left);
    output_used = buffer.size() - output_left;
  ...
  if (!ok || input_left != 0) return false;
  output.assign(buffer.data(), output_used);
  return true;
}
#endif
...
#else
  const char* encoding = encoding_from_code_page(code_page);
  std::string bytes;
  if (encoding == nullptr || !iconv_convert("UTF-8", encoding, text, bytes)) {
...
bool tw_decode(Runtime& runtime, const Value* args, uint32_t argc, Value& out,
  ...
  const char* encoding = encoding_from_code_page(code_page);
  std::string text;
  if (encoding == nullptr || !iconv_convert(encoding, "UTF-8", bytes, text)) {
