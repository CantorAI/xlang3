...
- [~] `sys`: `modules`, `exc_info`, stdio objects, argv/orig_argv/path/import-cache containers,
  ...
  live global `PY_START`/`PY_RETURN` callback dispatch with CPython-style callback arguments
  ...
  full CPython startup flags/config/runtime internals and remaining live PEP 669 event/local dispatch
  pending
