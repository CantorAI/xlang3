...
- [~] `zipfile`: native `ZipFile`/`ZipInfo`/`ZipExtFile` facade with stored/deflated archive `is_zipfile`, `namelist`, `infolist`, `getinfo`, `read`, `open` read handles, `write`, `writestr`, `mkdir`, `extract`, `extractall`, `testzip`, `close`, context managers, comments, common `ZipInfo` metadata, file-like archive objects, and keyword argument basics; encrypted ZIP, ZIP64, BZIP2/LZMA, write-mode `ZipFile.open`, `zipfile.Path`, `PyZipFile`, and exact CPython edge cases pending
