...
- Aligned `time` tuple-consumer type validation with CPython: `mktime()`,
  `strftime()`, and `asctime()` now reject plain list inputs with the
  `Tuple or struct_time argument required` `TypeError`, while preserving
  list/sequence acceptance for `time.struct_time(...)` construction.
...
- [~] `sys`: `modules`, `exc_info`/`exception` with nested handler-state
  ...
  including `None`/`bytes` event names, keyword `TypeError` diagnostics, and
  text signatures,
  ...
  metadata including trace/profile helper docs and text signatures plus
  `setswitchinterval` docs, `sys.int_info`
  ...
  frame helper docs and frame/runtime/cache helper text signatures including CPU-count, remote-debug,
  cache-clear/type-descriptor helper docs, and finalization helpers and Windows
  ...
  keyword diagnostics,
  CPython 3.14 startup metadata/hooks including exact fixture-covered CPython-shaped full `__doc__`,
  ...
  `f_builtins`, `_getframe` with CPython-style arity/type/keyword diagnostics,
  `_getframemodulename` with CPython-style `depth=` keyword binding and
  arity/type diagnostics, negative-depth and bool-depth handling plus too-shallow-stack
...
- [~] `time`: `time`, `time_ns`, `monotonic`, `monotonic_ns`, `perf_counter`, `perf_counter_ns`, `process_time`,
  ...
  diagnostics, CPython-style named-field `__repr__` through `repr()`/`str()` plus
  ...
  ordering for bad-directive, stray-percent, generic mismatch, trailing-input,
  and bounded numeric directive diagnostics, ISO directive validation failures,
  `%Y`/`%G` zero-year range rejection;
...
- [~] `fnmatch` / `glob`: real CPython 3.14 `Lib/fnmatch.py` and `Lib/glob.py`
  run over native VFS/path and `_sre` dependencies; `fnmatch`, `fnmatchcase`,
  `filter`, `filterfalse`, `translate`, `glob.has_magic`, `glob.escape`, and
  VFS-backed `glob`/`iglob` with recursive `**`, iterator-returning `iglob`,
  `root_dir`, `include_hidden`, and bytes-path result preservation are covered.
  Generic native-call dispatch now avoids trace/profile allocation when hooks
  are disabled, keeping cached source-level match callables fast; real `dir_fd`,
